# BFS218 Field Guide (sync Commons)
...
This site and the Online Asynchronous sibling (`bfs218-atlas`, the Companion) share one engine and one content set. Pushing either repo propagates the shared parts (`app.js`, `data/corpus-data.js`, `data/bfs218-mc.js`, `walkthroughs/`) to the other via `BFS218/_shared/propagate.sh` (installed as a pre-push hook by `_shared/install_bidi_hooks.sh`). Each site keeps its own skin (`index.html`), its `course.mode` label, its favicon, and its README.
