...
(function () {
  ...
  function header() {
    return '<header style="position:sticky;top:0;z-index:40;height:62px;background:#fff;border-bottom:2px solid var(--red);display:flex;align-items:center;padding:0 22px;gap:14px;flex:none">'
      ...
      + '<div class="soc-head-brand" style="display:flex;align-items:center;gap:10px;flex:none;min-width:0"><img src="./seneca-logo.png" alt="Seneca Polytechnic" style="height:34px;width:auto;display:block"><span class="soc-head-title" style="font-weight:600;font-size:1.0625rem;color:var(--ink);letter-spacing:0;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">BFS218</span></div>'
      + (D.course.mode ? '<span class="mono soc-head-mode" style="margin-left:auto;font-size:.75rem;font-weight:600;color:#474C57;background:#EFF1F4;padding:5px 10px;border-radius:6px;flex:none">' + esc(D.course.mode).toUpperCase() + '</span>' : '')
      + '<span class="mono soc-head-term" style="' + (D.course.mode ? '' : 'margin-left:auto;') + 'font-size:.75rem;font-weight:600;color:#B02318;background:#F6E3E1;padding:5px 10px;border-radius:6px;flex:none">FALL 2026</span>'
      + '</header>';
  }
  function sidebar() {
    var s = state;
  ...
  function compare() {
    ...
    var html = '<div class="rise"><div style="display:flex;align-items:baseline;gap:12px;margin-bottom:6px;flex-wrap:wrap"><h1 style="font-size:1.75rem;font-weight:600;margin:0">Hold them side by side</h1><span style="font-size:.9375rem;color:#474C57">' + (recs.length ? recs.length + ' of 3 selected' : 'choose 2 or 3') + '</span>'
      ...
      + '<p style="font-size:.9375rem;color:#474C57;margin:0 0 22px;">Choose readings from the list on the right, up to three, and they appear side by side here. Holding two readings together shows how they connect on the same topic.</p>';
  ...
  function weekPage(w, d) {
    ...
    var vid = VID ? '<section id="wk-vid" class="node"><h2 class="wk-sec">This week in 80 seconds</h2>'
      ...
      + (VID.vtt ? '<track kind="captions" srclang="en" label="English" src="./' + VID.vtt + '">' : '')
      + '</video>'
      + (VID.summary ? '<p style="margin:13px 0 0;font-size:.95rem;color:var(--ink-dim);line-height:1.6;">' + esc(VID.summary) + '</p>' : '')
    ...
    return '<div class="rise">' + hero + '<div class="wk-grid"><section>' + vid + pre + purpose + outcomes + guiding + programLens + concepts + terms + readings + watch + programCase + act + reflect + sg + kc + navRow + '</section>' + rail + '</div></div>';
  }
