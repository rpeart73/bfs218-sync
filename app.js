...
var ROUTES=[
  ...
  {id:'walkthrough',label:'Weekly Walkthrough',url:'https://rpeart73.github.io/bfs218-atlas/walkthroughs/',icon:'layers',external:true},
...
function renderNav(active){
  var nav=ROUTES.map(function(r){
    var on=r.id===active;
    var href=r.url?esc(r.url):r.hash;
    var attrs=r.url?'target="_blank" rel="noopener"':'aria-current="'+(on?'page':'false')+'"';
    var ext=r.url?'<span style="flex:none;font-size:.85em;color:#8a909c" aria-hidden="true">&#8599;</span>':'';
    return '<a href="'+href+'" '+attrs+' style="display:flex;align-items:center;gap:11px;border-radius:10px;padding:10px 12px;font-size:.9375rem;font-weight:'+(on?'600':'500')+';background:'+(on?'#EEF1F5':'transparent')+';color:'+(on?'#15171C':'#474C57')+';text-decoration:none;margin-bottom:2px"><span style="display:flex;align-items:center;justify-content:center;width:22px;height:22px;flex:none;color:'+(on?'var(--red)':'#8a909c')+'">'+ic(r.icon,19)+'</span><span style="flex:1">'+esc(r.label)+'</span>'+ext+'</a>';
  }).join('');
...
function weekView(n){
  ...
  var s=
    ...
    sec('slideshow','Interactive Slideshow', '<a class="btn btn-primary" href="https://rpeart73.github.io/bfs218-atlas/walkthroughs/" target="_blank" rel="noopener">Open the Weekly Walkthrough &#8599;</a><p class="muted" style="margin-top:10px;font-size:.9rem">An interactive, self-paced deck: a quick check of what you know, the week ideas with narration, then a closeout check. Opens in a new tab.</p>'+'<h3 style="margin-top:18px">Narrated walkthrough</h3>'+videoBlock(wk)+'<div class="notebar" style="margin-top:12px"><b>Pause and notice.</b> As you move through the slides, stop on one that surprises you and ask: who does this system assume I am, and who does it leave out?</div>')+
...
function cmpSynth(items){
  ...
  return 'Put '+list+' side by side and test them against one real technology, like a hiring tool, a face recognition system, or a predictive model. Then ask three simple questions. Do these ideas point at the same problem from different angles? Does one of them show a harm the others hide, a history, or a person the system was not built for? And what would you miss if you used only one? This course asks you to hold them together: technology is never neutral, and neither is the data it learns from.';
}
