...
(function () {
  ...
  function journeyHome() {
    ...
    var hero = '<section class="jhero jfade" style="margin-bottom:26px">' + heroArt()
      ...
      + '<p style="font-size:1.18rem;line-height:1.55;color:var(--ink);margin:0 0 20px;font-weight:500">The live class is where the ideas get worked out together. This site is your companion for everything before and after it: the readings, the walkthroughs, the practice, and your notes, all in one place, ready when class ends.</p>'
  ...
  function pathwaysPage() {
    var route = function (kind, title, subtitle, steps) {
      return '<section class="path-route path-' + kind + '"><div class="path-route-head"><div class="mono">' + esc('YOUR WEEKLY RHYTHM') + '</div><h2>' + esc(title) + '</h2><p>' + esc(subtitle) + '</p></div>'
    ...
    return '<div class="rise path-page">'
      + '<section class="path-hero"><div><div class="mono">COURSE PATHWAYS</div><h1>The site around your class</h1><p>Our class meets live each week. This route is how the site works around it, so class time goes deeper and nothing gets lost afterwards.</p></div><div class="path-compass" aria-label="Two pathway options"><span>BEFORE</span><b>prepare here</b><i></i><span>CLASS</span><b>go deeper live</b><i></i><span>AFTER</span><b>consolidate here</b></div></section>'
      + '<section class="path-summary"><div><span class="path-mark">1</span><b>Same course</b><p>Same readings, activities, assignments, and expectations.</p></div><div><span class="path-mark">2</span><b>Class-connected</b><p>Prepare before class, go deeper in class, consolidate after.</p></div><div><span class="path-mark">3</span><b>One weekly record</b><p>Both routes end with Generate Your Weekly Notes.</p></div></section>'
      + '<div class="path-routes">' + route('sync', 'Your weekly rhythm', 'Use the site around class so class time goes deeper.', syncSteps) + '</div>'
      + '<section class="path-close"><h2>Where to start</h2><p>Open the week you are in before class and skim the overview and readings. After class, come back to consolidate: the activity, the checks, and your notes. Use walkthroughs from inside the week whenever you want an idea explained again.</p><div class="path-actions"><button type="button" class="primary" onclick="SOC.station(2)"><b>Enter Week 2 Learning Room</b><small>Start the full module.</small></button><button type="button" onclick="SOC.assignmentPage(\'program\')"><b>Understand your Program\'s Connection to the Course</b><small>Preview field examples.</small></button><button type="button" onclick="SOC.careerChoices()"><b>Career Choices</b><small>See how BFS218 connects to your path.</small></button><button type="button" class="guide" onclick="SOC.assignmentPage(\'assignments\')"><b>Starting Your Assignment</b><small>Plan the graded work.</small></button></div></section>'
      + '</div>';
  }
  ...
  function overviewPage(w) {
    ...
    var how = '<section id="wk-how" class="node"><h2 class="wk-sec">How this course works</h2>'
      + '<p style="margin:0 0 10px;font-size:1rem;line-height:1.6">Each teaching week opens a module with its readings, a short interactive activity, and optional practice, a study guide and a knowledge check, that are never graded and never recorded. Our class meets live each week, and this site carries everything around it. A Study Week (October 26 to 30) falls between Weeks 7 and 8, a break with no new work, and nothing is due in the final week. Blackboard remains the official Seneca course platform for announcements, submissions, discussions, grades, course records, and administrative functions.</p>'
  ...
  function keyDatesCalendar() {
    return '<section class="node kd-cal" aria-label="Key dates for this course">'
      ...
      + '<p style="font-size:.9rem;line-height:1.55;color:var(--ink-dim);margin:0 0 12px">Deadlines follow the course rhythm: the Compass Check in Week 7, Canadian Case File in Week 8, Design the Repair and the Map Exchange close in Week 12, then the final project on Monday, December 7. Nothing is due in Study Week or the final week. Week-based deadlines land on your class day; Blackboard confirms the exact time.</p>'
  ...
  function assignmentsData() {
    return [
      ...
      },
      {
        ...
        weight: '20 marks (20%)',
        timing: 'Due Monday, December 7, 2026. Nothing is due in the final week.',
        opens: 'Week 8',
        release: '2026-11-02',
        due: 'Due by Monday, December 7, 2026. Nothing is due in Week 14.',
  ...
  function assignmentSummaryPanel() {
    return '<section class="asg-summary" aria-label="Assignment overview"><div><span>5 assignments</span><b>Each is worth 20%</b><small>Together they build one Personal Cartography.</small></div><div><span>Due dates</span><b>Week by week</b><small>Compass Check Week 7; Canadian Case File Week 8; Design the Repair and the Map Exchange close Week 12; Personal Cartography December 7. Nothing is due in Study Week or the final week.</small></div></section>';
  }
  ...
  function walkMount() {
    ...
    ov.innerHTML = '<button type="button" class="walk-theme" onclick="SOC.walkPanel()" aria-expanded="' + (!!_walk.panel) + '" aria-controls="walk-access-panel">Accessibility</button>'
      ...
      + walkPanelHtml()
      + '<div class="walk-stage" aria-live="polite"><div class="walk-slide wkslide-' + s.kind + '" key="' + i + '" tabindex="-1">' + walkSlideHtml(s, _walk.week) + '</div></div>'
  ...
  function walkOpen(w) {
    ...
    _walk = { week: w, i: 0, slides: slides, fig: null, panel: false, focusSlide: true, returnFocus: document.activeElement, bodyOverflow: document.body.style.overflow };
  ...
  function walkCloseDom() {
    var ret = _walk && _walk.returnFocus, oldOverflow = _walk && _walk.bodyOverflow;
    walkSpeakStop();
    var ov = document.getElementById('walk-overlay');
    if (ov) ov.remove();
  ...
  window.SOC = {
    ...
    walkNav: function (dir) { if (!_walk) return; var n = Math.max(0, Math.min(_walk.slides.length - 1, _walk.i + dir)); if (n === _walk.i) return; walkSpeakStop(); _walk.i = n; _walk.focusSlide = true; walkMount(); announce('Slide ' + (n + 1) + ' of ' + _walk.slides.length + '.'); },
    walkGoto: function (k) { if (!_walk) return; walkSpeakStop(); _walk.i = Math.max(0, Math.min(_walk.slides.length - 1, k)); _walk.focusSlide = true; walkMount(); announce('Slide ' + (_walk.i + 1) + ' of ' + _walk.slides.length + '.'); },
